...
std::ofstream Radio::pcapFile;
bool Radio::doPcapLog;
...
void Radio::initialize()
{
	...
	stateAfterTX = defaultStateAfterTX;
	...
	switch (msg->getKind()) {
		...
		case RADIO_CONTINUE_TX:{
			if (!radioBuffer.empty()) {
				...
				stateAfterTX = defaultStateAfterTX; // return to a default behaviour
			}
			break;
		}
...
void Radio::finishSpecific()
{
	...
	if (stats.bytesTransmitted > 0)
	        collectOutput("TXed bytes", "TX bytes", stats.bytesTransmitted);
	...
	if (stats.bufferOverflow > 0)
        ...
        recordScalar("TXed pkts", stats.transmissions);
        recordScalar("TXed bytes", stats.bytesTransmitted);
        if(self == 0) {

                if(pcapFile.is_open())
                    pcapFile.close();

                unsigned int numNodes = getParentModule()->getParentModule()->getParentModule()->par("numNodes");
                unsigned long long sumPkts = 0;
                ...
                for(unsigned int i = 0; i < numNodes; i++) {
                    ...
                    if(r) {
                        sumPkts += r->stats.transmissions;
                        sumBytes += r->stats.bytesTransmitted;
                    }
                }
                recordScalar("Total TXed pkts", sumPkts);
                recordScalar("Total TXed bytes", sumBytes);
            }

	if (stats.RxReachedNoInterference > 0)
		recordScalar("RX with NO interference", stats.RxReachedNoInterference);
	if (stats.RxReachedInterference > 0)
		recordScalar("RX despite interference", stats.RxReachedInterference);
	if (stats.RxFailedNoInterference > 0)
		recordScalar("RX Failed with NO interference", stats.RxFailedNoInterference);
	if (stats.RxFailedInterference > 0)
		recordScalar("RX Failed with interference", stats.RxFailedInterference);
	if (stats.RxFailedNoRxState > 0)
		recordScalar("RX Failed, non RX state", stats.RxFailedNoRxState);
	if (stats.RxFailedSensitivity > 0)
		recordScalar("RX Failed, below sensitivity", stats.RxFailedSensitivity);
	if (stats.RxFailedModulation > 0)
		recordScalar("RX Failed, wrong modulation", stats.RxFailedModulation);
	if (stats.bufferOverflow > 0)
		recordScalar("Buffer overflow", stats.bufferOverflow);
}
...
double Radio::popAndSendToWirelessChannel()
{
	...
	stats.transmissions++;
	stats.bytesTransmitted += macPkt->getByteLength();
	trace() << "Sending Packet, Transmission will last " << txTime << " secs";

	// log packet to PCAP file
	// TODO: Only logs TSTP packets
	if(doPcapLog) {
	    TSTPMACPacket * pkt = dynamic_cast<TSTPMACPacket*>(macPkt);
	    if(pkt) {
	        PCAP::Packet_Header packet_header(simTime().inUnit(-6), macPkt->getByteLength());
	        pcapFile << packet_header;
	        for(unsigned int i = 0; i < macPkt->getByteLength(); i++)
	            pcapFile << pkt->getData(i);
	    }
	}
...
void Radio::readIniFileParameters(void)
{
	...
	if (startingState.compare("RX") == 0)
        ...
        string st = par("stateAfterTX");
        defaultStateAfterTX = RX;
        if (st.compare("RX") == 0)
                defaultStateAfterTX = RX;
        else if (st.compare("TX") == 0)
                opp_error("Invalid state after TX name '%s'\n", st.c_str());
        else if (st.compare("SLEEP") == 0)
                defaultStateAfterTX = SLEEP;
        else
                opp_error("Unknown state after TX name '%s'\n", st.c_str());


        if(self == 0) {
            string pcap_file_name = par("pcapFile");
            if(pcap_file_name.size() > 0) {
                doPcapLog = true;
                pcapFile.open(pcap_file_name, std::ofstream::out | std::ofstream::trunc);
                PCAP::Global_Header global_header(_SYS::IEEE802_15_4::MTU, PCAP::Link_Type::IEEE802_15_4);
                pcapFile << global_header;
            } else
                doPcapLog = false;
        }
