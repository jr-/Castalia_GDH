...
void Timer::adjust(const Offset & o) { _offset += o; }

Timer::Time_Stamp Timer::adjust(double t0, double t1, double t2, double t3) {
    double ret = t3 * _frequency_error_divider - _offset;

    _offset = ((t2 - t1) - (t3 - t0)) / (t0 - t2);
    _frequency_error_divider = (t1 + _offset) / t0;

    return ret + _offset / _frequency_error_divider;
}

Timer::Timer(unsigned int unit) : _offset(0), _frequency_error_divider(1.0), _handler(0), _unit(unit), _watchdog_period(1, SIMTIME_S), _average_clock_error(0), _n_clock_error_registers(0)
{
