...
public:
    ...
    // Group Diffie-Hellman setup first node Security Bootstrap Control Message
        ...
        public:
            ...
            : Control(GDH_SETUP_FIRST, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _next(next), _parameters(parameters), _group_id(group_id){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_SETUP_INTERMEDIATE, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _next(next), _parameters(parameters), _group_id(group_id){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_SETUP_LAST, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _next(next), _parameters(parameters), _group_id(group_id){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_SETUP_LAST, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _next(next), _group_id(group_id), _finished(finished){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_ROUND, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _group_id(group_id), _round_key(round_key){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_BROADCAST, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _group_id(group_id), _round_key(round_key){ }
...
//            }
        ...
        public:
            ...
            : Control(GDH_RESPONSE, 0, 0, 0, Coordinates(0,0,0), Coordinates(0,0,0)), _destination(destination), _group_id(group_id), _round_key(round_key){ }
...
//                _nic->send(buf);
    ...
    public:
        Timekeeper(TSTP * tstp) : _tstp(tstp), _disable_explicit_sync(false), _total_error(0), _max_error(0), _n_adjusts(0), _first_pass(true) {
        ...
        bool synchronized() {
            ...
            if((!ret) && (now() > _next_sync + Radio::Timer::us2count(SYNC_PERIOD)))
    ...
    private:
        bool forward(Buffer * buf) {
            ...
            int d = buf->sender_distance - buf->my_distance;
            unsigned int aux = abs(d);
            if(aux > RADIO_RANGE)
                return false;

            if(expiry_metric) {
                TSTP::Time best_case_delivery_time = EXPIRY_BETHA * ((buf->my_distance + RADIO_RANGE - 1) / RADIO_RANGE * _tstp->_nic->period()) / 1000000;
                TSTP::Time relative_expiry = _tstp->destination(buf).t1 - _tstp->now();
        ...
        void offset(Buffer * buf) {
            ...
            if(old_expiry_metric) {
                ...
                if(best_case_delivery_time) {
                    buf->offset *= relative_expiry - best_case_delivery_time;
                    buf->offset /= best_case_delivery_time;
                }
            } if(effort_metric) {
                if(_tstp->_nic->statistics().tx_packets) {
                    buf->offset *= _tstp->_nic->statistics().tx_packets - _tstp->_nic->statistics().tx_relayed;
    ...
    public:
        Security(TSTP * tstp) : _key_manager(false), _dh_requests_open(0), _tstp(tstp) {
    ...
    private:
        ...
        bool unpack(const Peer * peer, unsigned char * msg, const unsigned char * mac, Time reception_time) {
            ...
            memset(nonce, 0, 16);
            memcpy(nonce, &reception_time, min(sizeof(Time), 16lu));
            poly.stamp(key, nonce, mi, MI_SIZE);
    ...
    private:
        ...
        Group_Diffie_Hellman::Round_Key _GDH_key; //final GDH key
        GDH_State _GDH_state;
        GDH_Node_Type _GDH_node_type;
        Simple_List<Region::Space> _GDH_next;
    };
    ...
    private:
        int life_keeper() {
            _tstp->setTimer(_tstp->LIFEKEEPER_TIMER, simtime_t(PERIOD, SIMTIME_US));
            if(!(_tstp->_locator->synchronized()
                    && _tstp->_timekeeper->synchronized()
                    && _tstp->_router->synchronized()
                    && _tstp->_security->synchronized())) {
                _tstp->trace()
...
public:
    ...
    static Coordinates sink(unsigned int unit = 0) { return Coordinates(0,0,0); } // TODO //return *_sinks[unit]->object(); }
